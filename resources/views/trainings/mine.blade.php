...
@section('content')
    <div class="page-title-box-sm" xmlns="http://www.w3.org/1999/html">
        <ul class="nav page-title" >
            <li class="nav-item"><label style="margin-top: 8px"><strong>{{ __('TRAINING FILTER') }}:</strong></label></li>
                ...
                <div class="col-md-6">
                    <div class="card shadow-sm border-left border-success" style="border-width: 5px!important;">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-10">
                                    <h3>{{ $training->getValue('training_name') }}</h3>
                                    <p>{{ $training->getValue('training_short_note') }}</p>
                                    <p class="mt-4 mb-0 font-12"><i class="dripicons-user mr-2 font-16 attribute-label" role="button" title="Domaćin"></i>{{ $training->getValue('training_host') }} </p>
                                    <p class="mt-0 mb-0 font-12"><i class="dripicons-location mr-2 font-16 attribute-label" role="button" title="Mesto održavanja"></i>{{ $training->getValue('location') }}</p>
                                    ...
                                    <p class="mt-0 font-12"><i class="dripicons-clockwise mr-2 font-16 attribute-label" role="button" title="Trajanje"></i>
                                        {{ $training->getText('training_duration') }} {{ $training->getText('duration_unit') }}
                                    </p>
                                </div>
                                <div class="col-2 text-center">
                                    @switch($training->getValue('training_type'))
                                        @case(1)
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    @if($training->getValue('training_type') == 1)
                                        <span class="p-1 attribute-label">
                                            @if($training->getClients()->count() > 0)
                                                {{ $training->getClients()->first()->getValue('name') }}
                                            @else
                                    ...
                                    @else
                                        <span class="border border-gray p-1 font-10 shadow-sm text-muted">
                                            @if($training->getValue('interests') == 0)
                                                {{__('All')}}
                                            @else
                                                {{ $training->getText('interests') }}
                                            @endif
                                        </span>
                                    @endif
