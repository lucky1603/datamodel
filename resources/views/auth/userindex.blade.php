...
@section('content')
    ...
    @foreach($profiles as $profile)
        <h5 class="mt-2 p-1 user-group-header" style="display: flex">
            <span class="float-left flex-grow-1">{{ $profile->getValue('name') }}</span>
            <a href="{{ route('user.addforprofile', $profile->instance->id) }}" class="text-light edituser" data-toggle="modal" data-target="#dialogHost"><i class="mdi mdi-text-account float-right flex-grow-0"></i></a>
        </h5>
        <table class="table table-striped">
            @php
