<div class="container">
    ...
    <div>
        <form id="myFormContract" method="POST" enctype='multipart/form-data' action="" class="h-100 pl-3 pr-3">
            ...
            <div class="row overflow-auto" >
                <div class="col-12 pt-3">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-3">
                            <input type="text" class="form-control form-control-sm" id="{{ $attribute->name }}"
                                   name="{{ $attribute->name }}"
                                   @if($attribute->getValue() != null) value="{{ $attribute->getValue() }}" @endif @if($status == -1) disabled @endif>
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <label for="{{ $attribute->name }}" class="col-sm-3 attribute-label col-form-label col-form-label-sm">{!! $attribute->label !!}</label>
                        <div class="col-sm-3">
                            <select id="{{$attribute->name}}" name="{{$attribute->name}}" class="form-control" @if($status == -1) disabled @endif>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-3">
                            <input type="text" class="form-control form-control-sm" id="{{ $attribute->name }}"
                                   name="{{ $attribute->name }}" value="{{ $attribute->getValue() }}" @if($status == -1) disabled @endif>
                        </div>
                        ...
                        @if($attribute->getValue() != null && $attribute->getValue() != ['filelink' => '', 'filename' => ''])
                            <div class="row">
                                ...
                                <div class="col-sm-9 mt-0 pt-0">
                                    ...
                                    <i id="iconDeleteContract" class="mdi mdi-delete font-24 attribute-label ml-2 @if($status == -1) d-none @endif " role="button" title="Obriši dokument"></i>
                                </div>
                            </div>
                        @endif
                        @if($attribute->getValue() == null || $attribute->getValue() == ['filelink' => '', 'filename' => ''])
                            <label class="col-form-label col-form-label-sm attribute-label mt-2">Priloži dokument ugovora</label>
                            <input type="file" class="form-control @if($status == -1) d-none @endif" id="{{ $attribute->name }}" name="{{ $attribute->name }}" >
                        @endif
            ...
            <div class=" @if($status == -1) d-none @else d-flex align-items-center justify-content-center @endif mt-4 mb-4" >
        ...
        <hr/>
        <program-statistics-form :profile_id="{{ $program->getProfile()->getId() }}" class="m-0 mt-2"></program-statistics-form>
    </div>
</div>
