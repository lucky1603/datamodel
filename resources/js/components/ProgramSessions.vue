<template>
  <div class="card w-100" style="height: 100%">
    ...
    <div v-else class="card-body">
      <img
        src="/images/custom/sizif.jpg"
        style="position: absolute; left: 0px; top: 0px; width: 100%; opacity: 0.3"
      />
      <div style="position: absolute; left: 100px; top: 100px">
        <p
          class="text-light"
          style="font-family: 'Roboto Light'; font-size: 38px; font-weight: bold"
        >
          NEMA IZABRANIH MENTORA
        </p>
      </div>
      <button
        type="button"
...
export default {
  ...
  methods: {
    async getMentors() {
      await axios.get(`/mentors/forprogram/${this.programid}`).then((response) => {
